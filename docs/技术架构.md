...
flowchart LR
  ...
  bash["bash tool"] --> shell["ctx.shell"]
...
默认 `SandboxBashProvider` 在 macOS 用 Seatbelt 包装 `bash -c` 后再 spawn。没有后端时返回 `SANDBOX_UNAVAILABLE`，不会退回裸执行。沙箱约束的是进程实际副作用，工具前置策略约束的是调用意图，两者不能互相替代。
...
- MCP 不是完整传输实现；沙箱只有 macOS Seatbelt。
