...
test("结构化方案渲染成带图解、发布助手和一键复制的单文件 HTML", async () => {
  ...
  expect((article.match(/data:image\/png/g) ?? []).length).toBe(3);
  ...
  const png = await readFile(join(root, "wechat", "journey.png"));
  ...
  expect(publishCopy.split("## 封面提示词")[1]).toContain("纸面手绘风格");
  expect(publishCopy.split("## 封面提示词")[1]).not.toContain("我为什么重新写了一遍 Agent 的运行骨架");
  expect(publishCopy).not.toContain("朋友圈文案");
});

test("兼容常见错字段名，一次收齐全部校验错误", async () => {
  const root = await mkdtemp(join(tmpdir(), "lob-wechat-alias-"));
  const registry = createDefaultToolRegistry();
  installWechatArticle(registry, { root });
  const paragraph = "真正难的不是把模型接进来，而是让一次执行留下可恢复的事实。事件日志记录发生过什么，投影负责决定模型现在看见什么，这个边界让失败可以复盘，也让架构取舍能够被普通开发者理解。";
  const aliased = await registry.execute("wechat_create_article", {
    outputDir: "wechat",
    plan: {
      title: "我为什么重新写了一遍 Agent 的运行骨架",
      subtitle: "从长期交付回到一线代码，我想补上的不是另一个聊天框，而是失败之后还能继续的运行时。",
      ...
      sections: [
        { heading: "演示结束后，问题才开始", paragraphs: [paragraph, paragraph], takeaway: "能恢复，才算真正开始交付。" },
        { heading: "开源迫使我把经验说清楚", paragraphs: [paragraph, paragraph] },
      ],
      quote: "我想开源的不是答案，而是一套能被运行、质疑和修改的判断过程。",
      closing: [paragraph, "你更在意 Agent 的能力上限，还是失败后的恢复能力？"],
      publish: {
        titleCandidates: ["为什么我要重新理解 Agent 运行时", "事件日志如何让 Agent 重新开始", "从长期交付到写下开源运行骨架"],
        summary: "一篇关于 Agent 运行时、事件日志与职业选择的开源手记。",
        tags: ["Agent", "开源"],
        moments: "把多年交付里的边界意识，写进一个可以回放和恢复的 Agent 骨架。",
        coverPrompt: "纸面手绘风格，以事件日志到状态投影的流程图为主体，使用项目 Logo，不要机器人和二维码。",
      },
      evidence: [
        { claim: "会话事实保存在只追加事件流中", file: "src/session-store.ts:30" },
        { claim: "模型消息始终从历史事件重新投影", file: "src/session.ts:138" },
        { claim: "回放与恢复行为有独立测试验证", file: "test/replay.test.ts:47" },
      ],
    },
  }, new AbortController().signal);
  ...
  const bundled = await registry.execute("wechat_create_article", {
    ...
    plan: {
      ...
      sections: [
        { title: "演示结束后，问题才开始", paragraphs: [paragraph, paragraph] },
        { title: "开源迫使我把经验说清楚", paragraphs: [paragraph, paragraph] },
      ],
      quote: "我想开源的不是答案，而是一套能被运行、质疑和修改的判断过程。",
      closing: { summary: paragraph, question: "你更在意 Agent 的能力上限，还是失败后的恢复能力？" },
  ...
  expect(bundled.output).toContain("title 长度必须是 12～60，当前 2 字");
  expect(bundled.output).toContain("publish.abstract 长度必须是 20～120，当前 2 字");
});
